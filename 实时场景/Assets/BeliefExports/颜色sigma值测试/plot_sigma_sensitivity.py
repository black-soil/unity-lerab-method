...
def calculate_composite_score(analysis_result, max_steps):
    ...
    score = (0.6 * fb) + (0.2 * (1 - cs / max_steps)) + (0.2 * (st / 30))  # 稳定性除以一个基准值
...
# 请根据您的实际情况修改这个列表
assumed_sigmas = [ 10, 25, 40, 55,70, 85, 100]  # 必须与json_files顺序对应
...
# 图表装饰
ax.set_xlabel('颜色相似度阈值 (σ)', fontsize=12, fontweight='bold')
ax.set_ylabel('综合信念识别率', fontsize=12, fontweight='bold')
ax.set_title('颜色因子σ超参数敏感性分析', fontsize=14, fontweight='bold', pad=20)
