...
public class ColorSigmaSensitivityExperiment : MonoBehaviour
{
    [System.Serializable]
    ...
    void Start()
    {
        ...
        {
            sigmaTests.Add(new SigmaTestConfig { testName = "Sigma_70", sigmaValue = 70f });
            sigmaTests.Add(new SigmaTestConfig { testName = "Sigma_85_Baseline", sigmaValue = 85f });
            sigmaTests.Add(new SigmaTestConfig { testName = "Sigma_100", sigmaValue = 100f });
        }
    ...
    private void ConfigureSigmaTest(SigmaTestConfig config)
    {
        ...
        {
            bayesObserver.ColorVariance = config.variance; // 传入方差值
            Debug.Log($"已设置颜色参数: σ={config.sigmaValue}, σ²={config.variance:F0}");
        }
        
        // 确保只启用颜色因子（在Inspector中预先设置好）
        Debug.Log($"实验配置: 目标={assumedTrueGoal}, 使用方差 σ²={config.variance:F0}");
    }
    ...
    {
        // 导出信念历史（会自动生成带时间戳的JSON文件）
        bayesObserver.ExportBeliefHistoryToJson();
        
        // 保存一份带有σ值标记的摘要
        string summary = $"颜色σ灵敏度测试: {config.testName}\n" +
                        $"参数: σ={config.sigmaValue}, σ²={config.variance}\n" +
        ...
        string filename = $"sigma_test_{testIndex+1:00}_{config.testName}.txt";
        File.WriteAllText(Path.Combine(directory, filename), summary);
        
        Debug.Log($"📄 测试摘要已保存: {filename}");
    }
